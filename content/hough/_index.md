...
A video demonstration can be found [here](https://www.youtube.com/watch?v=mXqz-rAfwIw).
...
As a simple example, algorithms such as edge detection that rely on convolutional kernels can be accelerated with dedicated hardware 
matrix multipliers.
...
We can also represent lines in a polar form. Instead of describing a line with a slope $m$ and an intercept $b$,
...
Hough Transforms work by going through each pixel in a source image and counting up "contributions" or "votes" of said pixel to a given line.
...
The contributions of a single pixel to our line Hough space are shown in Figure 5.

{{ figure(src="point-hough-space.png", caption="Figure 5: Contributions of a single point in our Hough space.", width=500, height=500) }}
...
value than any other point in the space. This point represents the $\rho$ and $\theta$ of the entire line in the source image.
Figure 6 shows our input image with a line at $y=50$ and Figure 7 shows the corresponding Hough space.
...
Our Hough Transform can be broken up into the following steps at a high level:
...
Furthermore, we aimed to minimize the amount of data transferred over Altera's Avalon Bus IP from the FPGA to the HPS, as having 
...
For debugging purposes we also used the board's built-in switches to change the behavior of our hardware without having to recompile
...
Using a VGA driver implemented on hardware enabled us to manipulate our image on our FPGA and directly output said image. It also let us 
be sure that we were not being bogged down by any overhead introduced from proprietary Qsys implementations/communication over the 
Avalon Bus.
...
These tweaks allow the local maxima finder to avoid bound checks, by making the usable space of our memory start at the first column and first row, as opposed to the zeroth.
...
The entire module begins accumulating at the falling edge of the `reset` signal.
...
Degree's increased resolution compared to radians allowed us to perform this simplification without losing accuracy in our transformer.
...
Figure 5 shows the contributions of a single pixel to a Hough space. In the figure, the faint line seen corresponds to
...
The actual accumulator interface is shown below in Figure 11:

{{ figure(src="accumulator-interface.png", caption="Figure 11: The interface of our accumulator module.", width=500, height=500) }}
...
{{ figure(src="edge_detection_ip.png", caption="Figure 14: Edge Detection IP Block from Altera Video IP Core", width=500, height=500) }}
...
{{ figure(src="working_verilog.png", caption="Figure 17: VGA screen output", width=500, height=500) }}
...
Finally, some `C++` intricacies required us to modify the assignment of some elements into arrays. In particular, using `assign()`
on some of the converted `std::vector`s did not deep copy the elements passed in, and use of an `=` assignment was necessary to overcome 
this limitation.
...
At this point, we have an SRAM containing our source image accessible in hardware, along with an SRAM containing all zeroes, except
for pixels on lines found in our original source image.
Our VGA driver combines these 2 values (performing a simple sum) before displaying them on our screen. (Figure 17).

At this point we have successfully found lines on a source image using a Hough Transform!!
...
The results of which can be seen in Figure 18. Note that this differs from Figure 7 which displays the expected result,
while Figure 18 displays the result of our hardware (with a halved $\rho$ resolution).

{{ figure(src="modelsim-hough-space.png", caption="Figure 18: The accumulation of our accumulator being input
a single vertical line.", width=500, height=500) }}

Seeing a strong correlation between the expected and actual results of our accumulator we could focus on our toplevel structures and FSMs.

We were able to visualize the Hough space of our accumulator in real time. An example of which can be seen in Figure 19. 
...
{{ figure(src="hough-visual.png", caption="Figure 19: Realtime visualization of our accumulated Hough space.", width=500, height=500) }}
...
and long compilation times in particular. Multiple times we encountered issues where compilation began, (breaking) changes
...
to two different SRAMs, made keep track of modules and wires difficult.
...
center pixels of the 5 x 5 image. This allows for neighbors of the physical point $(1,1)$ to be checked, as $(0,1)$ and $(1,0)$ are
guaranteed to be in bounds.

#### Validation of Sub-Blocks
The Hough Transforms computations are fairly involved and require nimbly navigating a range of binary representations over different spaces. This made it difficult to validate each sub-block in a timely manner. For instance, it was only by considerable effort on Nate's part that we were able to validate our compute block, requiring that he implement a Python version of the Hough Transform, run it on a simple image, pass that into a Python accumulator, and extract the results. The 2D input then had to be converted into our fixed-point binary representation (11.7, not standard), passed into a Verilog testbench, extracted (another 2D output), again converted from our representation, and passed into Python.

Issues like this arose for essentially all of our sub-blocks, and in some sense simply come with the territory. However, an effort might have been made ahead of time to explicitly map out the units, dimensions and representations of each step in the computation process to allow for better discretization of the design.


## Results

### Timing
We were able to demonstrate a visually noticeable and quantifiable acceleration of the Hough Transform. We tested the speed of our accelerator on several images, with varying complexity. In general, the more edges in an image, the more points needed to be traversed in the acculumator and the longer it took.

We were able to acquire extremely precise timing data by breaking out our `done` flag to the PIO ports and using an oscilloscope to measure the period, shown below for three test cases of varying complexity.
...
| Image    | Scope   | 
| -------- | ------- |
| {{ figure(src="image_1.png", caption="", width=250, height=250) }} | {{ figure(src="scope_1.png", caption="", width=250, height=250) }} |
|{{ figure(src="Image_2.png", caption="", width=250, height=250) }} | {{ figure(src="scope_2.png", caption="", width=250, height=250) }} |
| {{ figure(src="image_3.png", caption="", width=250, height=250) }}| {{ figure(src="scope_3.png", caption="", width=250, height=250) }}|

</center>

Shown in order, the first image contains a scene with a single shifted rectangle, used as our "moderate complexity" case, the second image shows a mess of cables, our "high complexity" case and the third image is a blank sheet of paper, our "low complexity" case.

Respectively, our accelerator takes approximately 5ms for our moderately complex case, 25ms for our complex case, and 1.5ms for our simple case.

Shown below in Figure 20 is the performance of both the software implementation and the hardware implementation, measured in compute time (per image) over each test case.


{{ figure(src="compute_time_chart.png", caption="Figure 20: Comparison between Hardware and Software Implementation of Stages of Line Hough Transform.", width=500, height=500) }}
...
No class on hardware acceleration would be complete without highlighting the fact that big-O notation, while a useful tool for building intuition, doesn't tell the whole story. Implementation details can mean a world of difference. In our case we see a worst-case (ie; Image 2) of 4 FPS vs 40 FPS! 
...
Since we were able to directly measure the time it took to run our hardware implementation on a range of real images, we consider our data to be incredibly precise and accurate. For the software implementation, we used the on-board linux system's clock to measure the time before and after each block of code. This allowed for better granularity in the results (which was important for deciding which aspects of the Hough Transform to port to hardware) at the expense of a bit of accuracy and precision.
...
We were not able to identify any safety concerns with our project. However, in order to validate our design, a number of images were printed and placed before the camera. Users should take care to minimize risk of exposure to paper-cuts by utilizing gloves, goggles and perhaps a hardhat.

### Usability
The nature of our project is inherently visual, meaning some consideration needs be given for visually impaired users. However, given that our system would mostly likely see use as a sub-block in a larger automated system, we did not identify any particular issues with usability.
...
One of the most cycle-intensive computations is computing the rho from a given x, y, and theta, requiring multiple multiplications and adds, likely taking multiple cycles to compute a rho for each pixel. On the FPGA, this is easily parallelized and performed in a single cycle.
...
### Future Work
It would be interesting to expand our Hough Transformer to detect more than just lines. Algorithms [exist](https://en.wikipedia.org/wiki/Generalised_Hough_transform) that detect arbitrary shapes within an image, and our initial goal was to implement such a detector.
It seems likely that we would need to find a way around our memory constraints in order to achieve such as an algorithm, as
detecting more complex shapes requires higher dimensional Hough spaces, which would need to be stored in ever larger accumulator SRAMs.
