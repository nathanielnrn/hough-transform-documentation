...
center pixels of the 5 x 5 image. This allows for neighbors of the physical point
$(1,1)$ to be checked freely, as $(0,1)$ and $(1,0)$ are
guaranteed to be in bounds, even though the physical point $(1,1)$ represents the point $(0,0)$ in our original image.
...
In the end we were able to obtain a dramatic qualitative increase in quality of our Hough Transform.
We went from a slow, jittery, aliased version that ran at approximately 5 fps, to a near-real time version
that performed just as accurately. We were especially pleased with the smoothness of our final result.

It seems that a lot of time could have been saved if we were more disciplined with our Verilog wiring.
However, for the most part, we were very pleased with gradual the progress we made.
...
We used Altera IP for much of our design. Much of our `C` code was based on OpenCV under
the [3-clause BSD license](https://github.com/opencv/opencv/blob/4.4.0/LICENSE).
We are grateful for Bruce and Hunter's available software libraries and hardware designs.
...
**Brandon:** Collision detection, voltage divider, aggregation algorithm, website, and report.

**Nathaniel:** Porting OpenCV functions, accumulator testing, website, and report.

**Tomas:** Random number generation, motion-random-distribution effects, serial, website, and report.
